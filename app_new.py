...
DATASET_OPTIONS = {
    "Dataset A(Linear)": os.path.join("datasets", "Dataset A(Linear).csv"),
    "Dataset B(Seasonal)": os.path.join("datasets", "Dataset B(Seasonal).csv"),
    "Dataset C(Nonlinear)": os.path.join("datasets", "Dataset C(Nonlinear).csv"),
}
DEFAULT_DATASET_NAME = "Dataset A(Linear)"
...
st.sidebar.markdown('<div class="sidebar-control-label">Choose Stored Dataset</div>', unsafe_allow_html=True)
selected_dataset_name = st.sidebar.selectbox(
    "Choose Stored Dataset",
    list(DATASET_OPTIONS.keys()),
    index=list(DATASET_OPTIONS.keys()).index(DEFAULT_DATASET_NAME),
    label_visibility="collapsed"
)
selected_dataset_path = DATASET_OPTIONS[selected_dataset_name]

sample_df = load_sample_data(selected_dataset_path)

if uploaded_main:
    df_raw = pd.read_csv(uploaded_main)
    data_source = format_dataset_name(uploaded_main.name)
else:
    df_raw = sample_df
    data_source = selected_dataset_name
